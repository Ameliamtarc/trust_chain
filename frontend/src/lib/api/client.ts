// Same-origin requests are proxied to the backend by next.config.ts, avoiding browser CORS.
...
export async function readChainProject(projectId: string) {
  const response = await fetch(`${apiBase}/chain/projects/${encodeURIComponent(projectId)}`, { cache: 'no-store' });
